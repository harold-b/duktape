...
DUK_INTERNAL void duk_debug_write_tval(duk_hthread *thr, duk_tval *tv) {
	...
	case DUK_TAG_BOOLEAN:
		...
		duk_debug_write_byte(thr, DUK_TVAL_GET_BOOLEAN(tv) ? DUK_DBG_IB_TRUE : DUK_DBG_IB_FALSE);
		break;
...
DUK_LOCAL void duk__debug_handle_inspect_heap_object(duk_hthread *thr, duk_heap *heap) {
	...
	/* FIXME: this will now detach on error, maybe send ERR instead */
	...
	case DUK_HTYPE_OBJECT: {
		...
		duk__debug_inspect_prop_bool(thr, "extensible", DUK_HOBJECT_HAS_EXTENSIBLE(h_obj));
		duk__debug_inspect_prop_bool(thr, "constructable", DUK_HOBJECT_HAS_CONSTRUCTABLE(h_obj));
		duk__debug_inspect_prop_bool(thr, "bound", DUK_HOBJECT_HAS_BOUND(h_obj));
		duk__debug_inspect_prop_bool(thr, "compiledfunction", DUK_HOBJECT_HAS_COMPILEDFUNCTION(h_obj));
		duk__debug_inspect_prop_bool(thr, "nativefunction", DUK_HOBJECT_HAS_NATIVEFUNCTION(h_obj));
		duk__debug_inspect_prop_bool(thr, "bufferobject", DUK_HOBJECT_HAS_BUFFEROBJECT(h_obj));
		duk__debug_inspect_prop_bool(thr, "thread", DUK_HOBJECT_HAS_THREAD(h_obj));
		duk__debug_inspect_prop_bool(thr, "array_part", DUK_HOBJECT_HAS_ARRAY_PART(h_obj));
		duk__debug_inspect_prop_bool(thr, "strict", DUK_HOBJECT_HAS_STRICT(h_obj));
		duk__debug_inspect_prop_bool(thr, "notail", DUK_HOBJECT_HAS_NOTAIL(h_obj));
		duk__debug_inspect_prop_bool(thr, "newenv", DUK_HOBJECT_HAS_NEWENV(h_obj));
		duk__debug_inspect_prop_bool(thr, "namebinding", DUK_HOBJECT_HAS_NAMEBINDING(h_obj));
		duk__debug_inspect_prop_bool(thr, "createargs", DUK_HOBJECT_HAS_CREATEARGS(h_obj));
		duk__debug_inspect_prop_bool(thr, "envrecclosed", DUK_HOBJECT_HAS_ENVRECCLOSED(h_obj));
		duk__debug_inspect_prop_bool(thr, "exotic_array", DUK_HOBJECT_HAS_EXOTIC_ARRAY(h_obj));
		duk__debug_inspect_prop_bool(thr, "exotic_stringobj", DUK_HOBJECT_HAS_EXOTIC_STRINGOBJ(h_obj));
		duk__debug_inspect_prop_bool(thr, "exotic_arguments", DUK_HOBJECT_HAS_EXOTIC_ARGUMENTS(h_obj));
		duk__debug_inspect_prop_bool(thr, "exotic_dukfunc", DUK_HOBJECT_HAS_EXOTIC_DUKFUNC(h_obj));
		duk__debug_inspect_prop_bool(thr, "exotic_proxyobj", DUK_HOBJECT_HAS_EXOTIC_PROXYOBJ(h_obj));
...
#endif  /* DUK_USE_DEBUGGER_INSPECT */
		case DUK_DBG_CMD_GARBAGECOLLECT: {
			/* FIXME: ifdef guard? */
			DUK_D(DUK_DPRINT("debug command unimplemented: %d", (int) cmd));
			duk_debug_write_error_eom(thr, DUK_DBG_ERR_UNSUPPORTED, "unimplemented command");
			break;
		}
		default: {
